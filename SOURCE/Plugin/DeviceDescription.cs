    ...
    public partial class DeviceDescription : UserControl
    {
       ...
       //     e.Node.ContextMenuStrip = contextMenuStripforInterfaceClassLib;
        ...
        private void openToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            openFile.Filter = "AML Files(*.aml; *.amlx;*.xml;*.AML;*.edz )|*.aml; *.amlx;*.xml;*.AML;*.edz";

            if (openFile.ShowDialog() == DialogResult.OK)
            {
                if (openFile.FileName != null)
                {
                    if (Convert.ToString(Path.GetExtension(openFile.FileName)) == ".edz")
                    {
                        ConverterAML converterAML = new ConverterAML();

                        //add path to generate amlx file
                        converterAML._pathAMLDestinationDirectory = Path.GetDirectoryName(openFile.FileName);
                        //function of class to export .edz file to .amlx
                        converterAML.exportStart(openFile.FileName);
                    ...
                    {
                        //send path to function to open file
                        openFiles(openFile.FileName);
                    }
                }
            }
        }
        ...
        private void openFiles(string file)
        {
            ...
            clear();
            device.filepath = Path.GetDirectoryName(file);
            ...
            try
            {
                ...
                if ((filetype = Convert.ToString(Path.GetExtension(file))) == ".amlx")
                {
                ...
                if ((filetype = Convert.ToString(Path.GetExtension(file))) == ".aml" || (filetype = Convert.ToString(Path.GetExtension(file))) == ".xml")
                {
            ...
            {
                this.Cursor = Cursors.Default;
                //    newToolStripMenuItem_Click(sender, e);        //---------------------------------------verify this other time----------------------------
        ...
        private void addNewItemToolStripMenuItem_Click(object sender, EventArgs e)
        {
               ...
               TreeNode node = treeViewInterfaceClassLib.SelectedNode;
            treeViewInterfaceClassLib.Nodes.Add("chave", "New node", 0);
        }
        ...
        {
            var dic = searchAMLLibraryFile.DictionaryForInterfaceClassInstancesAttributes;

            TreeNode node = treeViewInterfaceClassLib.SelectedNode;
            if (node.IsSelected)
            {
                node.Nodes.Add("chave", "new sub node", 1);
            }
